...
export default function OverviewDashboard({ userId }) {
    ...
    const fetchOverview = async () => {
      try {
        const [eventRes, taskRes] = await Promise.all([
          fetch(`/api/calendar/overview?userId=${userId}`),
  ...
  const taskStatuses = [
    ...
    { key: "ongoing", label: "Ongoing", color: "yellow" },
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
        ...
        {taskStatuses.map((status) => (
          <StatCard
            key={status.key}
            icon={<ListTodo className={`w-6 h-6 text-${status.color}-600`} />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <Card className="shadow-xl rounded-2xl border border-gray-100 bg-gradient-to-br from-white to-indigo-50">
        ...
        {/* Tasks Overview */}
        <Card className="shadow-xl rounded-2xl border border-gray-100 bg-gradient-to-br from-white to-blue-50">
          <CardHeader className="flex justify-between items-center flex-wrap gap-3">
            <CardTitle className="flex items-center gap-2 text-lg font-semibold text-blue-700">
              <ListTodo className="w-6 h-6 text-blue-600 shrink-0" />
              Tasks Overview
            </CardTitle>
          </CardHeader>
          ...
          <CardContent>
            ...
            >
              <TabsList className="hidden sm:grid sm:grid-cols-4 gap-2 bg-blue-50 p-1 rounded-lg mb-4">
                {taskStatuses.map((status) => (
                  <TabsTrigger
                    key={status.key}
                    value={status.key}
